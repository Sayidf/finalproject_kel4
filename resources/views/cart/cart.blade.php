      ...
      <div class="cart-desktop">
        <table class="table text-white table-borderless text-center align-middle" id="cart">
          <thead class="text-muted">
            <tr>
              ...
              <td class="text-start px-0">Menu Detail</td>
              <td>Price</td>
              <td>Quantity</td>
              <td>Subtotal</td>
              <td></td>
            ...
            @if (session('cart'))
              @foreach (session('cart') as $id => $details)
                ...
                <tr data-id="{{ $id }}" class="ajax-cart">
                  ...
                  <td data-th="Subtotal">Rp {{ number_format($details['harga'] * $details['quantity'], 0, ',', '.') }}
									</td>
                  <td class="text-end align-middle">
										<button class="btn remove-from-cart text-tiny btn-dark border-0 rounded-circle" title="Delete"><i class="fa fa-times"></i></button>
									</td>
                </tr>
              @endforeach
            @endif
        ...
        <div class="w-100 mt-5">
          <div class="d-flex justify-content-between">
            ...
            <div class="d-flex justify-content-between">
              <a href="{{ url('/menu') }}" class="btn btn-outline-light p-3 border-0 text-uppercase me-3"><i class="fa fa-chevron-left me-2"></i>Continue Shopping</a>
              @if (Auth::check())
              @empty($ar_reservasi)
              ...
              @else
                @if ($ar_reservasi->status == 'approved')
                  <form method="POST" action="{{ url('cart/checkout') }}">
                    @csrf
                    <button class="btn btn-dark p-3" data-bs-toggle="modal" data-bs-target="#checkout-modal">Checkout</button>
          ...
          <div data-id="{{ $id }}" class="row ajax-cart">
            ...
            <div class="col px-0 ps-2">
              <div class="d-flex align-items-start flex-column bd-highlight">
                ...
                <p class="fw-bold text-temp-primary text-tiny">Rp {{ number_format($details['harga'], 0, ',', '.') }}</p>
                <div class="mt-auto bd-highlight w-100">
                  <div class="d-flex justify-content-between w-100">
                    ...
                    <div class="d-flex justify-content-end">
                      <button class="btn minus text-white border-0"><i class="fa fa-minus text-tiny"></i></button>
                      <input type="text" value="{{ $details['quantity'] }}" class="quantity update-cart text-center text-tiny" />
                      <button class="btn plus text-white border-0"><i class="fa fa-plus text-tiny"></i></button>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div class="d-flex align-items-center checkout-btn">
                  ...
                  @else
                    @if ($ar_reservasi->status == 'approved')
                      <form method="POST" action="{{ url('cart/checkout') }}">
                        @csrf
                        <button type="submit" class="d-grid align-items-center" data-bs-toggle="modal"
