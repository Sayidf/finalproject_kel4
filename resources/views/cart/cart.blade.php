        ...
        @if (session('cart'))
          @foreach (session('cart') as $id => $details)
            ...
            <div data-id="{{ $id }}" class="row ajax-cart">
              <div class="col-4">
                @empty($details['foto'])
                  <img src="{{ url('/public/assets/img/menu/placeholder.jpg') }}" width="100" class="rounded-3 float-start px-0"/>
                @else
                  <img src="{{ url('/public/assets/img/menu') }}/{{ $details['foto'] }}" width="100" class="rounded-3 float-start px-0"/>
                @endempty
              </div>
              ...
              <div class="px-2">
                <button class="btn remove-from-cart text-white text-tiny border-0 px-1 py-3 fw-bold" title="Delete">Hapus</button>
